...
for i=1:length(img_path_list)
    ...
    disp('image_name=');
    disp(image_name);
    processed_name = sprintf("Deflare_Ycbcr_dsus_%s",image_name);
    disp(processed_name)
...
%     flag1 = avg_outter_1 - avg_inner_1>3;
    ...
    [output_color] = Suppression(cr_roi_ds,th);  
    lab_roi_a = cr_roi_ds;
    if flag0 && flag1
    ...
    cr_adjust = imresize(output_color_blend,8,'bicubic');
    ...
    output_adjust(end-895:end,:,:) = uint8(rgb_roi_adjust);
