function [lab_adjust_roi] = DeFlareCista_V2(inputfilename,omg_l,light_mode)
clear all;
close all;
inputfilename_split = strsplit(inputfilename,'.');
end_name = inputfilename_split(end);
if all(end_name{1}=='raw')
    %%
    imagename = './2.raw';
    ...
    input_awb2(:,:,1) = temp_results(:,:,1)*D(1);
    input_awb2(:,:,3) = temp_results(:,:,3)*D(3);
    input_awb2(:,:,2) = temp_results(:,:,2);
    %%
    % imwrite(final_results_awb,'2_awb.bmp');
    gauss_final_results_awb = imgaussfilt(input_awb,6);
    Process_Image = gauss_final_results_awb;
    % gauss_final_results_awb = gauss_final_results_awb;
else
    ori_image = imread(imagename);
    lab = rgb2lab(double(ori_image)/255);
    lab_roi = lab(:,1:900,:);
    input_awb = ori_image;
    Process_Image = input_awb;
end
%%  ------------- Process and judgement of Deflare image ------------%%

% figure,imshow(ori_image)
% lab_roi = rgb2lab(double(gauss_roi_awb)/255);
% lab_adjust = lab_roi;
% end_cell = name_splits(end)
