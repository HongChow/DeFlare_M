...
for i=1:length(img_path_list)
    ...
    disp('image_name=');
    disp(image_name);
    processed_name = sprintf("Deflare_Ycbcr_gain0.95_omg_0.05_%s",image_name);
    disp(processed_name)
...
%     flag1 = avg_outter_1 - avg_inner_1>3;
    ...
    th = th_gain*th;
    [output_color] = Suppression(cr_roi,th,omg);  
    lab_roi_a = cr_roi;
    if flag0 && flag1
    ...
    output_adjust(end-899:end,:,:) = uint8(rgb_roi_adjust);
